...
class RandomRotate:
    ...
    def __call__(self, img):
        ...
        img_rot = rotate(img, angle = angle, order = order, axes = (-2, -1), reshape = False)
...
class Binning:
    def __init__(self, block_size = 2, mask = None):
    ...
    def __call__(self, data):
        ...
        return self.binning(data, block_size) if mask is None else \
               self.binning_with_mask(data, mask, block_size)


    def binning(self, data, block_size = 2):
        """
        Return binned data.
        ...
        Args:
            ...
            block_size : int, size of blocks subject to binning.

        Returns:
            data_binned: numpy.ndarray, data after binning.

        """
        # Fetch the H and W of an image...
        B, H, W = data.shape

        # Pad data with edge values...
        # Purpose: ensure input data are evenly divisible by the block size
        # Second % to facilitate x if x < y else 0
        dH_padded = (block_size - H % block_size) % block_size
        dW_padded = (block_size - W % block_size) % block_size
        pad_width = (
            (0, 0        ), # No padding for B dimension (before, after)
            (0, dH_padded), # Pad with edge values along H dimension
            (0, dW_padded), # Pad with edge values along W dimension
        )
        data_padded = np.pad(data, pad_width = pad_width, mode = 'edge')

        # Reshape the padded data by introducing binning dimensions...
        H_binned = (H + dH_padded) // block_size
        W_binned = (W + dW_padded) // block_size
        data_padded = data_padded.reshape((B, H_binned, block_size, W_binned, block_size))

        # Calculate the block mean...
        data_binned = np.nanmean(data_padded, axis = (-3, -1))

        return data_binned


    def binning_with_mask(self, data, mask, block_size = 2):
        """
        Return binned masked data.
        ...
        """
        # Mask unwanted data with np.nan...
        data_masked = apply_mask(data, mask)

        # Binning...
        data_binned = binning(data_masked, block_size = block_size)
...
class RandomBrightness:
    def __init__(self, path_brigtness_distribution = None):
        self.path_brigtness_distribution = path_brigtness_distribution

        self.random_scale_function = self.init_random_scale_function()


    def init_random_scale_function(self):
        random_scale_function = lambda x: x    # A trivial function that returns input by default

        if self.path_brigtness_distribution is not None:
            # Add shot to shot fluctuation to data_train and data_validate...
            # Load photon count and probability
            photon_count_list, prob_list = np.load(self.path_brigtness_distribution)

            def constrained_random_scale_function(img):
            ...
            random_scale_function = constrained_random_scale_function

        return random_scale_function


    def __call__(self, img):
        return self.random_scale_function(img)
